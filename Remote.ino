...
// Các hàm điều khiển máy lạnh
void setupAC() {
  ac.begin();
  Serial.println("Khởi tạo máy lạnh Daikin...");
  ac.on();
  ac.setFan(0);  // Auto fan
  ac.setMode(kDaikin176Cool);  // Chế độ làm lạnh
  ac.setTemp(currentTemp);
  ac.send();
  Serial.println("Đã khởi tạo máy lạnh thành công");
}

void turnOn() {
  Serial.println("Đang BẬT máy lạnh...");
  ac.on();
  ac.setTemp(currentTemp);
  ac.send();
  Serial.println("Đã BẬT máy lạnh");
}

void turnOff() {
  Serial.println("Đang TẮT máy lạnh...");
  ac.off();
  ac.send();
  Serial.println("Đã TẮT máy lạnh");
}

void setTemperature(int temp) {
  if (temp >= 18 && temp <= 30) {
    Serial.printf("Đang đặt nhiệt độ: %d°C\n", temp);
    currentTemp = temp;
    ac.setTemp(currentTemp);
    ac.send();
    Serial.println("Đã cập nhật nhiệt độ thành công");
  } else {
    Serial.println("Lỗi: Nhiệt độ phải từ 18°C đến 30°C");
  }
}
...
void setup() {
  ...
  if (Firebase.ready()) {
    ...
    // Khởi tạo trạng thái LED ban đầu trên Firebase
    Serial.println("8. Khởi tạo trạng thái LED trên Firebase...");
    if (Firebase.RTDB.setBool(&fbdo, "led/status", false)) {
      Serial.println("✓ Đã khởi tạo trạng thái LED trên Firebase");
    } else {
      Serial.println("✗ Lỗi khởi tạo LED: " + fbdo.errorReason());
    }
